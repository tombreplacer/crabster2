...
- **CI**: Автоматическая проверка кода и сборка при каждом коммите (Linux и Windows).
...
2. Закоммитьте изменения:
   ...
   git push
   ```
3. Создайте тег и отправьте его на GitHub:
   ```bash
   git tag v0.1.2
   git push origin v0.1.2
   ```
   После этого GitHub Action автоматически соберет проект и создаст страницу релиза.

### Как пересоздать тег (перезапустить релиз):

Если релиз собрался с ошибкой или нужно обновить файлы без смены версии:
```bash
# 1. Удалить тег локально
git tag -d v0.1.2

# 2. Удалить тег на GitHub (это удалит сам релиз)
git push origin --delete v0.1.2

# 3. Создать тег заново и отправить
git tag v0.1.2
git push origin v0.1.2
```
